...
export const WEATHER_CODES = (code: number, hour?: number): Record<string, string> | undefined => {
   
    const isNight:boolean = hour ? hour > 20 || hour < 6 : false;
...
export const WINDSPEED = (value: number): Record<string, string> => {
   if(value === 0) {
    return {
        title: 'Calm',
