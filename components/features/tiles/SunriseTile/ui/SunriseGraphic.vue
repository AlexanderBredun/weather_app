<template>
    <div ref="canvasSunsetHolder" class="h-52 w-full overflow-hidden relative sunset-holder"
        :style="{ '--sunriseW': beforeMidnight(data.sunrise), '--sunsetW': 1 - beforeMidnight(data.sunset) }">
        ...
        <div class="sun absolute flex justify-center items-center" :class="{ 'moon': !(sunsetCalc >= 0 && sunsetCalc <= 1) }"
            :style="{ '--sunPos': beforeMidnight(data.timeNow) }">
...
const sunsetCalc = computed((): number => {
    const res = +(1 - ((new Date(props.data.sunset).getTime() - new Date(props.data.timeNow).getTime()) / (new Date(props.data.sunset).getTime() - new Date(props.data.sunrise).getTime()))).toFixed(2);
    return res;
})

const beforeMidnight = (time: string): number => {
    const mid = new Date(props.data.timeNow);
